...
## المجموعة 13: إدارة العملاء (المرحلة السادسة)

> ثلاث طبقات متمايزة عمداً: **المهتمّ** اهتمام لم يُتحقَّق منه، و**العميل** جهة تعامل
> فعلي، و**الفرصة** صفقة محتملة. خلط المهتمّ بالعميل يجعل «عدد العملاء» رقماً بلا
> معنى، وهو رقم يبني عليه صاحب المشروع قراره بالتوسّع.

### `crm_customers`
| العمود | ملاحظات |
|---|---|
| `code` | فريد **داخل المنشأة** لا عبر المنصة؛ يُولَّد `CUS-0001` إن تُرك فارغاً |
| `tax_number` | الرقم الضريبي وحده يُجمع لأن الفاتورة تحتاجه — **لا رقم قومي ولا بيانات بنكية** (§10) |
| `credit_limit` | **تنبيه لصاحب المشروع لا قاعدة تمنع**؛ المنصة لا تحجب بيعاً ولا تصنّف جدارة |
| `linked_user_id` | ربط اختياري بحساب على المنصة إن كان العميل مسجَّلاً |

الرصيد المستحقّ **غير مخزَّن**: يُحسب من `erp_invoices` لحظة القراءة، فلا يفترق عن
الفواتير عند أول إلغاء أو حذف إيصال. وعميل عليه مستحقّات **لا يُؤرشَف**.

### `crm_contacts`
أشخاص الجهة العميلة. الجهة تدوم والأشخاص يتغيّرون، فلا يصحّ دمجهما.
`is_primary` واحدة لكل عميل — تعيين ثانية يُلغي الأولى في المعاملة نفسها.

### `crm_leads`
| العمود | ملاحظات |
|---|---|
| `status` | `new`·`contacted`·`qualified`·`converted`·`lost` |
| `converted_customer_id`·`converted_at` | أثر التحويل — لا تُفقد قصة العلاقة عند أول بيع |
| `source_enquiry_id` | استفسار السوق الذي وُلد منه المهتمّ |
| `lost_reason_ar` | **إلزامي** عند الفقد |

**التحويل مرة واحدة**، ولا تُكتب حالة `converted` يدوياً: تُكتب بإنشاء العميل في
المعاملة نفسها، وتنتقل معها أنشطة المهتمّ إلى ملفّ العميل.

### `crm_opportunities`
`stage` = `new`→`qualified`→`proposal`→`negotiation`→`won`|`lost`. الانتقالات محكومة،
فالقفز من «جديدة» إلى «مكسوبة» يُخفي أين تتعثّر الصفقات فعلاً. **الخسارة تستوجب سبباً**،
وإعادة الفتح تمحو `closed_at`.

`expected_value` و`probability` **تقديرات صاحب المشروع** لا تنبّؤ من المنصة.

### `crm_activities`
سجلّ واحد لما جرى (`occurred_at`) ولما سيجري (`due_at`). فصلهما في جدولين كان سيجعل
«آخر تواصل مع العميل» استعلامين ودمجاً يدوياً. مهمة `planned` بلا موعد مرفوضة —
لا تذكّر بشيء.

---

## المجموعة 14: إدارة الموارد المبسّطة (المرحلة السادسة)

> ⚠ **ليست محاسبة.** لا قيد مزدوج ولا دليل حسابات ولا أجور ولا تخطيط إنتاج — كلها
> خارج نطاق النسخة صراحةً (§14). ما هنا سجلّ تشغيلي على **الأساس النقدي**، ومخرجاته
> تقارير إدارية يجب أن يراجعها محاسب مؤهّل.

### `erp_items`
| العمود | ملاحظات |
|---|---|
| `sku` | فريد داخل المنشأة |
| `quantity_on_hand` | **رصيد مشتقّ لا مُدخَل** — يُكتب فقط من `InventoryService::record()` |
| `track_stock` | الخدمة لا مخزون لها؛ لا تُقبل حركات على صنف غير متتبَّع |
| `cost_price` | آخر تكلفة شراء معروفة، تُحدَّث عند الاستلام |
| `listing_id` | ربط اختياري بإعلان السوق — عند الربط يصير **الصنف مصدر الرصيد الوحيد** للإعلان |

إيقاف التتبّع لصنف رصيده ليس صفراً **مرفوض**: يترك كمية معلّقة بلا دفتر.

### `erp_stock_movements`
**دفتر مضاف إليه فقط: لا تعديل ولا حذف.** التصحيح حركة تسوية جديدة تحمل سببها.

| العمود | ملاحظات |
|---|---|
| `movement_type` | `opening`·`purchase`·`sale`·`return_in`·`return_out`·`adjustment`·`damage`·`transfer_out`·`production_in` |
| `quantity_delta` | موجب للوارد وسالب للمنصرف — **الاتجاه محكوم بالنوع لا بإشارة من المتصفّح** |
| `balance_after` | الرصيد لحظة الحركة، مثبَّت ليبقى الدفتر مقروءاً دون إعادة حساب التاريخ |
| `reference_type`·`reference_id` | `order`·`invoice`·`purchase_order`·`manual` |
| `reason_ar` | **إلزامي** للتسوية والتالف والتحويل |

الرصيد السالب مرفوض: يصف واقعاً مستحيلاً ويُفسد تقدير قيمة المخزون.
الدفتر نفسه **هو سجلّ التدقيق للمخزون** — لا طبقة تدقيق ثانية قد تتناقض معه.

### `erp_invoices` · `erp_invoice_items`
`status` = `draft`·`issued`·`partially_paid`·`paid`·`cancelled`.

**المسودة تُحرَّر والمستند لا يُحرَّر.** بعد الإصدار يحمل العميل نسخة، فتعديل بند
يُنتج نسختين تحملان الرقم نفسه. التصحيح **إلغاء موثّق بسبب** يُعيد المخزون المصروف،
وفاتورة سُجّلت عليها مقبوضات **لا تُلغى**.

- `invoice_number` — تسلسلي **داخل المنشأة** (`INV-26-0001`)، لا عشوائي ولا عالمي.
- `amount_paid` — **مشتقّ** من `erp_payments`، لا يُحرَّر، والحالة تتبعه لا العكس.
- `customer_name_ar` وأسماء البنود وأسعارها **تُنسخ لحظة الإضافة**: تغيير سعر الصنف
  غداً لا يغيّر فاتورة صدرت بسعر اليوم.
- المجاميع تُحسب من البنود دائماً ولا تُقبل من النموذج.

**المسودة ليست بيعاً**: مستبعدة من تقارير المبيعات كالملغاة.

### `erp_payments`
**المنصة لا تحصّل شيئاً.** الإيصال تسجيل لمبلغ استلمه صاحب المشروع خارج المنصة (§14).
المقبوض **لا يتجاوز المتبقّي** — الزيادة تُنتج ذمماً سالبة في التقارير.
`receipt_number` تسلسلي داخل المنشأة (`REC-26-0001`).

### `erp_expenses`
على **الأساس النقدي**: ما خرج فعلاً بتاريخ خروجه، بلا استحقاق ولا إهلاك.
مصروف بتاريخ مستقبلي مرفوض. بند `salaries` **تدوين لما دُفع لا نظام أجور**.
`receipt_media_id` يشير إلى `media` — **صورة الإيصال ملف لا BLOB** (§10).

### `erp_suppliers` · `erp_purchase_orders` · `erp_purchase_order_items`
`status` = `draft`·`sent`·`partially_received`·`received`·`cancelled`.

**الاستلام هو ما يزيد المخزون، لا إصدار الأمر.** أمر أُرسل ولم تصل بضاعته ليس بضاعة
في المخزن. `quantity_received` يتراكم مع كل استلام جزئي **ولا يتجاوز المطلوب**،
والحالة تتبع البنود تلقائياً. أمر استُلم جزء منه **لا يُلغى**: البضاعة دخلت فعلاً،
وإلغاؤه يترك مخزوناً بلا مستند يفسّر مصدره.

---

## مخطط العلاقات (المرحلة السادسة)

```
organizations ──< crm_customers ──< crm_contacts
                        │
                        ├──< crm_opportunities ──< crm_activities
                        └──< erp_invoices ──< erp_invoice_items ──> erp_items
                                    └──< erp_payments

crm_leads ──(تحويل)──> crm_customers        customer_enquiries ──> crm_leads

organizations ──< erp_items ──< erp_stock_movements
                     │              ↑ (كل تغيّر في الرصيد يمرّ من هنا)
                     └──> listings  (ربط اختياري: الصنف مصدر الرصيد الوحيد)

organizations ──< erp_suppliers ──< erp_purchase_orders ──< erp_purchase_order_items
                        └──< erp_expenses ──> media (صورة الإيصال)
```

---

## المجموعة 15: المحتوى (المرحلة السابعة)

> **الكتابة ليست النشر.** الجداول الثلاثة تشترك في آلة حالة واحدة:
> `draft` → `pending_review` → `published` | `rejected`، وأعمدة `published_by`
> و`published_at` تُسجّل **من** قرّر النشر ومتى. التعديل بعد النشر يُعيد المادة
> للطابور ويمسح ختمها، والرفض يمسحه كذلك.
...
| العمود | ملاحظات |
|---|---|
| `slug` | مُعرّف نصّي فريد يُشتقّ من العنوان ويُرقَّم عند التصادم |
| `excerpt_ar` | يظهر في القوائم والمشاركة؛ يُشتقّ من أول النصّ إن تُرك فارغاً |
| `body_ar` | HTML **مُنقّى عند الحفظ** بقائمة سماح (`HtmlSanitizer`) لا عند العرض |
| `review_note_ar` | سبب الرفض يصل للمحرّر — **إلزامي عند الرفض** |
| `published_by`·`published_at` | تُمسحان عند الرفض أو التعديل، فلا تبدو المادة معتمدة |
| `reading_minutes` | يُقدَّر من عدد الكلمات إن لم يكتبه المحرّر |
| `is_demo` | وسم البيانات التجريبية (§15) — يظهر للزائر كوسم وتنويه |

`FULLTEXT` على العنوان والملخّص والنصّ. حدّ الرؤية العامة: `published` وغير محذوف،
مكتوب مرة واحدة في `ContentRepository::publicConstraint()`.

### `faqs`
`section` من ثمانية أقسام تتبع وحدات المنصة. تمرّ بالبوابة نفسها وبنفس أعمدة النشر.

### `static_pages`
صفحات المنصة التعريفية والقانونية (`about` · `terms` · `privacy`).

**تُحرَّر ولا تُنشأ ولا تُحذف:** `slug` مفتاح ثابت يعرفه الكود، وحذف صفحة الشروط يكسر
رابطاً في تذييل كل صفحة. البذرة **مرجعية لا تجريبية** فتعمل في الإنتاج، ووجود
الصفوف **شرط تشغيل** يفحصه `php bin/console health`.

`needs_legal_review` يُظهر للزائر تنويهاً بأن الصياغة أولية، ويُزال بعد اعتماد الجهة
القانونية المختصة.

---

## مخطط العلاقات (المرحلة السابعة)

```
categories (type='article') ──< articles ──> users (المحرّر · الناشر)
                                   └──> media (صورة الغلاف)

faqs ──> users (الناشر)          static_pages ──> users (المحرّر · الناشر)

audit_logs ──> users · organizations      (للقراءة فقط — لا مسار يعدّله)
```

---

## ملاحظة عامة على المخطط

جداول المنصة **84 جدولاً** موزّعة على خمس عشرة مجموعة. القواعد المشتركة بينها:

- كل جدول مملوك لمنشأة يحمل `organization_id`، والقراءة تفشل مغلقة بدونه.
- الحذف **ناعم** (`deleted_at`) حيثما كان للسجلّ أثر يجب ألّا يُفقد.
- كل قرار يحمل **من اتّخذه ومتى**، وكل رفض يحمل **سببه**.
- القيم المشتقّة (رصيد المخزون، المسدَّد من الفاتورة) تُحسب من مصدرها ولا تُحرَّر.
- الأسرار (`token_hash` والرموز) في أعمدة `ascii_bin` لا `utf8mb4_ci`.
